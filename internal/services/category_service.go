...
func (s *CategoryService) CreateCategory(category *models.Category) error {
	...
	existingCategories, _, err := s.repo.FindAll(1, 1, category.Slug)
	if err != nil {
...
func (s *CategoryService) GetAllCategories(page, pageSize int, search string) ([]models.Category, int64, error) {
	return s.repo.FindAll(page, pageSize, search)
}

func (s *CategoryService) UpdateCategory(id uint, updatedCategory *models.Category) error {
	...
	if updatedCategory.Slug != "" {
		...
		existingCategories, _, err := s.repo.FindAll(1, 1, updatedCategory.Slug)
		if err != nil {
			return fmt.Errorf("error checking existing category: %v", err)
		}
