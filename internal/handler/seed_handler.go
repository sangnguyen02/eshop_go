...
func (h *SeedHandler) SeedData(c *gin.Context) {
	...
	for parentName, subs := range subCategoryNames {
		...
		for _, subName := range subs {
			...
			subCategories = append(subCategories, model.Category{
				Name:        subName,
				Slug:        slug,
	...
	if err := tx.Model(&model.Category{}).Select("id").Find(&categoryIDs).Error; err != nil {
		tx.Rollback()
	...
	var brandIDs []uint
	if err := tx.Model(&model.Brand{}).Select("id").Find(&brandIDs).Error; err != nil {
		tx.Rollback()
	...
	for i := 1; i <= numProducts; i++ {
		...
		variants := make([]model.ProductVariant, numVariants)
		for j := 0; j < numVariants; j++ {
			color := colors[rand.Intn(len(colors))]
			size := sizes[rand.Intn(len(sizes))]
			variants[j] = model.ProductVariant{
		...
		reviews := make([]model.ProductReview, numReviews)
		for j := 0; j < numReviews; j++ {
			reviews[j] = model.ProductReview{
